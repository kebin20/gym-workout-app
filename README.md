...
Current production version: **v3.4.8**
...
- Rebuilt the in-app brand mark as a single, smoothly blended gradient surface with a smaller centered “L”, eliminating the layered-tile effect while keeping the install artwork full-bleed for native iOS masking.
- Added dedicated transparent-corner favicon assets so browser tabs use the same smooth rounded silhouette without changing the native iOS home-screen artwork.
